...
d_define_method_override(loader, job)(struct s_object *self) {
  ...
  struct s_object *resources_path = f_string_new_constant(d_new(string), d_game_resources),
                  *template_png = f_string_new_constant(d_new(string), d_game_resources_default_png),
                  *template_ttf = f_string_new_constant(d_new(string), d_game_resources_default_ttf),
                  *template_json = f_string_new_constant(d_new(string), d_game_resources_default_json),
                  *template_ogg = f_string_new_constant(d_new(string), d_game_resources_default_ogg),
                  *template_lisp = f_string_new_constant(d_new(string), d_game_resources_default_lisp);
  d_try{
    ...
    if ((loader_attributes->media_factory =
      ...
      d_assert(stream_configuration =
          d_call(loader_attributes->resources_json, m_resources_get_stream, d_game_game_configuration, e_resources_type_common));
      if ((json_configuration = f_json_new_stream(d_new(json), stream_configuration))) {
        d_assert(stream_ui = d_call(loader_attributes->resources_json, m_resources_get_stream, d_game_ui_configuration, e_resources_type_common));
...
d_define_method(loader, delete)(struct s_object *self, struct s_loader_attributes *attributes) {
  d_log(e_log_level_low, "teardown loader");
